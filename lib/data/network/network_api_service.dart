...
class NetworkApiService extends BaseApiService {
  ...
  Future bookingService(
    ...
    try {
      ...
      if(bookingType==0){
        var model=data as BookingServiceSendDataModel;
        List<http.MultipartFile> multipartFileList = [];

  print('date ${model.startTime}\serviceProviderServiceId:${model.date}');
        request.fields['serviceProviderServiceId'] = model.serviceProviderServiceId;
        request.fields['livingRoom'] = model.livingRoom;
        request.fields['bedRoom'] = model.bedRoom;
        request.fields['kitchen'] = model.kitchen;
        request.fields['bathRoom'] = model.bathRoom;
        request.fields['startTime'] = model.startTime;
        request.fields['date'] = model.date;
          request.fields['pincodeForService'] = model.pinCodeForService;
        request.fields['landmark'] = model.landmark;
        request.fields['addressForService'] = model.addressForService;
        for(int i=0;i<model.coordinates.length;i++){
          request.files.add(http.MultipartFile.fromString('coordinates', model.coordinates[i]));
        }
        multipartFileList.clear();
        print('image list is ${multipartFileList.length}');

        if(kIsWeb){
          for(int i=0;i<model.imagesBitesList.length;i++){
            multipartFileList.add(http.MultipartFile.fromBytes(
                'images', model.imagesBitesList[i],
                contentType: MediaType.parse("image/png"),
                filename: model.imagesNameList[i]));
          }
        }else {
          print('before image list is ${multipartFileList.length}');
          for(int i=0;i<model.imagesList.length;i++) {
            multipartFileList.add(await http.MultipartFile.fromPath(
              'images',
              model.imagesList[i],
              contentType: MediaType.parse("image/png"),
            ));
            print('after insert $i image list is ${multipartFileList.length}');

          }
        }
        print('end image list is ${multipartFileList.length}');
        request.files.addAll(multipartFileList);
      }else{
        print('Service not impleneting in Network api class');
      }
      print('hitting api');
      var res = await request.send();
      var result = await http.Response.fromStream(res);
